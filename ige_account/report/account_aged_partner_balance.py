...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        if ctx.get('account_filters'):
            partner_clause = 'AND (account_account.id IN %s)'
            arg_list += (tuple(ctx['account_filters']),)
        ...
        for i in range(5):
            ...
            if ctx.get('analytic_account_ids'):
                additional_clause += 'AND (l.analytic_account_id IN %s)'
                args_list += (tuple(ctx['analytic_account_ids'].ids),)

            if ctx.get('account_filters'):
                additional_clause += 'AND (account_account.id IN %s)'
                args_list += (tuple(ctx['account_filters']),)
        ...
        if ctx.get('analytic_account_ids'):
            additional_clause += 'AND (l.analytic_account_id IN %s)'
            arg_list += (tuple(ctx['analytic_account_ids'].ids),)
        ...
        for partner in partners:
            ...
            if partner['partner_id']:
                ...
                values['name'] = browsed_partner.name and len(browsed_partner.name) >= 45 and not self.env.context.get(
                    'no_format') and browsed_partner.name[0:41] + '...' or browsed_partner.name
